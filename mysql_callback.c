    ...
    if (bev_arg->remote || (bev_arg->ms && bev_arg->ms->not_need_remote)) {
        ...
        if (bev_arg->type == BEV_TARGET) {
            ...
            if (bev_arg->ms->handshake == 0) {
                if (handle_init_packet_from_server
            ...
            if (bev_arg->ms->handshake == 1) {
                if (handle_auth_packet_from_client
                    (bev_arg, bev, len, bev_remote)) {
                    return;
                }
            }
        ...
        if (evbuffer_get_length (bufferevent_get_output (bev_remote)) >=
            ...
            bufferevent_setcb (bev_remote, mysql_read_callback,
                               mysql_write_callback, mysql_event_callback,
                               (void *) bev_arg->remote);
        }
...
void
mysql_write_callback (struct bufferevent *bev, void *ptr)
{
    ...
    if (evbuffer_get_length (bufferevent_get_output (bev)) == 0) {
        if (bev_arg->remote) {
            ...
            bufferevent_setcb (bev, mysql_read_callback, NULL,
...
void
mysql_event_callback (struct bufferevent *bev, short events, void *ptr)
{
    struct bev_arg *bev_arg = ptr;
        ...
        if (events & BEV_EVENT_CONNECTED) {
            ...
            if (bev_arg->type == BEV_TARGET) {
                if (events & BEV_EVENT_ERROR) {
                    logmsg ("BEV_EVENT_ERROR (mysql_callback) dest: %s error: %s", bev_arg->destination->s, evutil_socket_error_to_string(evutil_socket_geterror(bufferevent_getfd(bev))));
            ...
            if (bev_remote) {
                ...
                if (evbuffer_get_length (bufferevent_get_output (bev_remote))
                    ...
                    free (bev_arg->remote);
                    bufferevent_setcb (bev_remote, mysql_read_callback,
                                       mysql_write_callback,
                                       mysql_event_callback,
                                       (void *) bev_arg->remote);
                }
            }
 ...
 */
void
cache_mysql_init_packet_read_callback (struct bufferevent *bev, void *ptr)
{
    ...
    if (len) {
        ...
        evbuffer_copyout (bufferevent_get_input (bev),
                          cache_mysql_init_packet, len);

        cache_mysql_init_packet_len = len;

        cache_mysql_init_packet_scramble =
            get_scramble_from_init_packet (cache_mysql_init_packet,
                                           cache_mysql_init_packet_len);
    }
...
void
cache_mysql_init_packet_event_callback (struct bufferevent *bev, short events,
                                        void *ptr)
{
    struct bev_arg *bev_arg = ptr;
...
void
mysql_connect_timeout_cb (evutil_socket_t fd, short what, void *arg)
{
